...
namespace SoulKnight2.Framework.UIManager
{
    public class UIManager : ModuleSingleton<UIManager>, IModule
    {
        ...
        private readonly Dictionary<string, string> loadedPath = new();
        ...
        {
            UIWindowBase panel = null;
            //检查是否已打开
            if (loadedWindows.TryGetValue(name, out panel))
            {
            ...
            //检查路径是否配置
            string path = "";
            if (!loadedPath.TryGetValue(name, out path))
            {
            ...
            if (!loadedPrefabWindows.TryGetValue(name, out panelPrefab))
            {
                panelPrefab = Resources.Load<GameObject>(path);
                loadedPrefabWindows.Add(name, panelPrefab);
            }

            GameObject instanceObject = UnityEngine.Object.Instantiate(panelPrefab);
            UIWindowBase instance = instanceObject.GetComponent<UIWindowBase>();
            ...
            panel = panelObject.GetComponent<UIWindowBase>();
            loadedWindows.Add(name, panel);
            panel.OnOpen(name);
